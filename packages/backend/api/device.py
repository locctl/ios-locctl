    ...
    try:
        ...
        raise HTTPException(
            ...
            detail={
                ...
                "message": (
                    f"偵測到 iOS {e.version},ios-locctl 自 v0.1.49 起僅支援 "
...
def _kill_stale_tunnel_processes() -> int:
    ...
    killed = 0
    # macOS: use pgrep + kill to find and terminate stale tunnel processes
    try:
        out = subprocess.check_output(
            ["pgrep", "-f", "wifi_tunnel.py"],
        ...
        # pgrep returns non-zero if no processes found — not an error
        pass
    except FileNotFoundError:
        _tunnel_logger.warning("pgrep not found; skipping stale tunnel cleanup")
    except Exception:
        _tunnel_logger.exception("Failed to enumerate stale tunnel processes")
    # Remove stale info file
    try:
        info_path = Path.home() / ".ios-locctl" / "wifi_tunnel_info.json"
        if info_path.exists():
            info_path.unlink()
    except OSError:
...
async def wifi_tunnel_start(req: WifiTunnelStartRequest):
    ...
    async with _tunnel.lock:
        ...
        if getattr(sys, "frozen", False):
            candidate = Path(sys.executable).resolve().parent.parent / "wifi-tunnel" / "wifi-tunnel"
            if candidate.exists():
        ...
        if bundled_tunnel is not None:
            ...
            script = Path(__file__).resolve().parent.parent / "wifi_tunnel.py"
